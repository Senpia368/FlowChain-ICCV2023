...
#     'PEDESTRIAN': {
#         'position': {
#             'x': {'mean': 5.04, 'std': 8.82},
#             'y': {'mean': 6.25, 'std': 14.17}
#         },
#         'velocity': {
#             'x': {'mean': -0.07, 'std': 1.45},
#             'y': {'mean': -0.16, 'std': 1.56}
#         },
#         'acceleration': {
#             'x': {'mean': -0.05, 'std': 1.84},
#             'y': {'mean': -0.09, 'std': 2.07}
#         }
#     }
# }
...
# Process ETH-UCY
# for desired_source in ['eth', 'hotel', 'univ', 'zara1', 'zara2', 'cuip_eth_all', 'cuip_eth_vehicle']:
for desired_source in ['cuip_eth_all', 'cuip_eth_vehicle']:
...
data_columns = pd.MultiIndex.from_product([['position', 'velocity', 'acceleration'], ['x', 'y']])
# for desired_source in ['stanford', 'cuip_sdd_all', 'cuip_sdd_vehicle']:
for desired_source in ['cuip_sdd_all', 'cuip_sdd_vehicle']:
    for data_class in ["train", "test"]:
        data_path = os.path.join(raw_path, desired_source, f"{data_class}_trajnet.pkl")
        print(f"Processing {desired_source} {data_class}")
        data_out_path = os.path.join(data_folder_name, f"{desired_source}_{data_class}.pkl")
        ...
        for scene, data in group:
            ...
            if desired_source == "stanford":
                data['frame'] = data['frame'] // 12
            elif desired_source.startswith("cuip_sdd"):
                data['frame'] = data['frame'] -- data['frame'].min()
